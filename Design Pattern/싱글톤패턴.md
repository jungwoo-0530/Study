...
- 스프링의 싱글톤은 스프링 컨테이너(Bean Factory / Applicaiton Context)에 의해 구현된다.
- 스프링에서는 컨테이너 내에서 특정 클래스에 대해 `@Bean`이 정의되면, **스프링 컨테이너는 그 클래스에 대해 딱 한 개의 인스턴스**를 만든다.
- Bean이 **호출**될 때마다 스프링은 생성된 **공유 인스턴스를 리턴**시킨다.
- 공유 인스턴스의 생성 시점은 스프링 컨테이너에 따라 달라진다.
  - Bean Factory : 최초 호출 시점에서 인스턴스를 생성(Lazy loading)
  - Application Context : 미리 모든 공유 인스턴스를 다 초기화해 두었다가 호출될 때 리턴
  - 참고로 스프링에는 빈의 생성과 관계설정 같은 제어를 담당하는 IoC 컨테이너인 Bean Factory가 존재한다. 하지만 실제로는 빈의 생성과 관계설정 외에 **추가적인 기능**이 필요한데, 이러한 이유로 스프링에서는 **빈 팩토리를 상속받아 확장한 Applicaiton Context**를 주로 사용.
- **Bean 관리의 주체인 스프링 컨테이너**는 그 어떤 호출에도 **단일 공유 인스턴스를 리턴**시키므로 **thread safe**도 자동으로 보장된다.